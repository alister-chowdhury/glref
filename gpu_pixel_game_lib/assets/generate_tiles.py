...
_SBSAR_DIR = os.path.abspath(
    os.path.join(__file__, "..", "src", "sbsar")
)

_STATIC_DIR = os.path.abspath(
    os.path.join(__file__, "..", "src", "static")
)

_TILES_DIR = os.path.abspath(
    os.path.join(__file__, "..", "tiles")
)
...
def _sbsrender_command(
    ...
    command = [
        ...
        "--output-name={0}_{{outputNodeName}}".format(texture_group_name),
         "--output-path={0}".format(_TILES_DIR)
    ]
...
def _dispatch_procedural(args):
    """Execute a process.
    
    Args:
        args (tuple(str, list[str])): Name of texture and cmd to execute
...
if __name__ == "__main__":
    ...
    cmds = (
        (name, cmd)
        for generator in procedral_generators
    ...
    for done in pool.map(_dispatch_procedural, cmds):
